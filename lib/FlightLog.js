...
module.exports = class FlightLog {
  ...
  smart_filter(){
    ...
    // Find all possible sets of non-colliding routes :/
    this.find_best_sets();
    // for each set, know how many flights are used and how many miles traveled
    // present best in each case and then with remaining flights show fewest possible chains
  }
  ...
  find_best_sets(){
    ...
    this.all_routes.forEach(route =>{
      if (route.findIndex(leg => group.flights[leg.id]) == -1) {
        // no collition so route is compatible with ones already in set
        group.routes.push(route);
        group = route.reduce((group, leg) => {
          group.flights[leg.id] = true;
          group.flight_count++;
